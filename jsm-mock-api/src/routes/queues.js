...
router.get('/servicedesk/:serviceDeskId/queue/:queueId/issue', (req, res) => {
  ...
  else if (queue.name === 'SLA Breached')
    issues = issues.filter(
      (i) =>
        i.fields.status.name !== 'Closed' &&
        (i.fields.priority.name === 'Blocker' || i.fields.priority.name === 'Critical'),
    );
  res.json({
    size: issues.length,
    start: 0,
    limit: 50,
    isLastPage: true,
    values: issues.map((i) => ({ id: i.id, key: i.key, fields: i.fields })),
  });
});
...
router.get('/request/:requestId/sla', (req, res) => {
  const issue = store.issues.find(
    (i) => i.key === req.params.requestId || i.id === req.params.requestId,
  );
    ...
    values: [
      {
        ...
        completedCycles: { breached: responseBreached ? 1 : 0 },
        ongoingCycle: {
          breached: responseBreached,
          elapsedTime: {
            millis: elapsedMinutes * 60000,
            friendly: `${Math.floor(elapsedMinutes)}m`,
          },
          remainingTime: {
            millis: Math.max(0, (policy.responseTimeMinutes - elapsedMinutes) * 60000),
            friendly: `${Math.max(0, Math.floor(policy.responseTimeMinutes - elapsedMinutes))}m`,
          },
          goalDuration: { millis: policy.responseTimeMinutes * 60000 },
          percentageElapsed: Math.min(100, (elapsedMinutes / policy.responseTimeMinutes) * 100),
        },
      },
      {
        name: 'Time to resolution',
        completedCycles: { breached: resolutionBreached ? 1 : 0 },
        ongoingCycle: {
          ...
          remainingTime: {
            millis: resolutionRemaining * 60000,
            friendly: `${Math.floor(resolutionRemaining)}m`,
          },
          goalDuration: { millis: policy.resolutionTimeMinutes * 60000 },
          percentageElapsed: resolutionPct,
        },
      },
    ],
  });
