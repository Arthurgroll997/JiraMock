...
router.get('/:issueIdOrKey/transitions', (req, res) => {
  ...
  const available = wf.filter(t => t.from === currentStatus).map(t => ({
    id: t.id,
    name: t.name,
...
router.post('/:issueIdOrKey/transitions', (req, res) => {
  ...
  if (!transition || !transition.id) return res.status(400).json({ errorMessages: ['transition.id is required'], errors: {} });
  const typeName = issue.fields.issuetype.name;
  const currentStatus = issue.fields.status.name;
  const wf = store.transitions[typeName] || [];
  const t = wf.find(t => t.id === transition.id && t.from === currentStatus);
  if (!t) return res.status(400).json({ errorMessages: [`Transition ${transition.id} is not valid from status "${currentStatus}"`], errors: {} });
