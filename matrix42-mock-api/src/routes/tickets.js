...
router.get('/', (req, res) => {
  ...
  if (search) {
    ...
    results = results.filter((t) =>
      `${t.Title} ${t.Description} ${t.TicketNumber}`.toLowerCase().includes(s),
    );
  }
...
router.post('/', (req, res) => {
  ...
  const ticket = {
    ID: uuidv4(),
    TicketNumber: `INC${String(count).padStart(3, '0')}`,
    Status: 'New',
    Priority: 3,
...
router.post('/:id/resolve', (req, res) => {
  ...
  if (req.body.resolution) {
    store.ticketComments.push({
      id: uuidv4(),
      ticketId: req.params.id,
      text: `Resolution: ${req.body.resolution}`,
      author: 'system',
      createdDate: new Date().toISOString(),
    });
  }
...
router.post('/:id/escalate', (req, res) => {
  ...
  if (req.body.reason) {
    store.ticketComments.push({
      id: uuidv4(),
      ticketId: req.params.id,
      text: `Escalation: ${req.body.reason}`,
      author: 'system',
      createdDate: new Date().toISOString(),
    });
  }
