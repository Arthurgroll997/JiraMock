...
router.get('/', (req, res) => {
  ...
  if (search) {
    ...
    results = results.filter((u) =>
      `${u.FirstName} ${u.LastName} ${u.AccountName} ${u.Email}`.toLowerCase().includes(s),
    );
  }
...
router.post('/', (req, res) => {
  ...
  const wf = {
    ...
    type: 'onboarding',
    userId: user.ID,
    userName: user.AccountName || `${user.FirstName}.${user.LastName}`.toLowerCase(),
    status: 'pending',
    createdDate: new Date().toISOString(),
    completedDate: null,
    params: { department: user.Department, manager: user.Manager },
  };
...
router.get('/:id/assets', (req, res) => {
  const assignments = store.assetAssignments.filter(
    (a) => a.userId === req.params.id && a.status === 'Active',
  );
...
router.delete('/:id/assets/:assetId', (req, res) => {
  const idx = store.assetAssignments.findIndex(
    (a) => a.userId === req.params.id && a.assetId === req.params.assetId && a.status === 'Active',
  );
...
router.get('/:id/tickets', (req, res) => {
  const user = store.objects.SPSUserClassBase.find((u) => u.ID === req.params.id);
  if (!user) return res.status(404).json({ error: 'User not found' });
  const tickets = store.objects.SPSActivityClassBase.filter(
    (t) => t.Reporter === user.AccountName || t.AssignedTo === user.AccountName,
  );
  res.json(tickets);
});
...
router.get('/:id/software', (req, res) => {
  const userAssets = store.assetAssignments.filter(
    (a) => a.userId === req.params.id && a.status === 'Active',
  );
