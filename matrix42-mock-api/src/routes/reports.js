...
router.get('/compliance', (req, res) => {
  ...
  const avgScore =
    compliance.length > 0
      ? Math.round(compliance.reduce((s, c) => s + c.complianceScore, 0) / compliance.length)
      : 0;
  res.json({
    totalScanned: compliance.length,
    compliant,
    nonCompliant,
...
router.get('/licenses', (req, res) => {
  const result = store.objects.SPSSoftwareType.map((sw) => {
    ...
    return {
      ...
      totalLicenses: sw.TotalLicenses || null,
      usedLicenses: installCount,
...
router.get('/user-access', (req, res) => {
  ...
  const matrix = users.map((u) => {
    ...
    const assetCount = store.assetAssignments.filter(
      (a) => a.userId === u.ID && a.status === 'Active',
    ).length;
    ...
    return {
      ...
      groups,
      assetCount,
      pendingRequests: accessReqs.filter((r) => r.status === 'pending').length,
    };
  });
  res.json(matrix);
});
