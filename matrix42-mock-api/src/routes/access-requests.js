...
router.post('/:id/approve', (req, res) => {
  ...
  if (ar.status !== 'pending')
    return res.status(400).json({ error: `Cannot approve request in status: ${ar.status}` });
  ar.status = 'approved';
...
router.post('/:id/deny', (req, res) => {
  ...
  if (!ar) return res.status(404).json({ error: 'Access request not found' });
  if (ar.status !== 'pending')
    return res.status(400).json({ error: `Cannot deny request in status: ${ar.status}` });
  ar.status = 'denied';
...
router.post('/:id/revoke', (req, res) => {
  ...
  if (ar.status !== 'approved')
    return res.status(400).json({ error: `Cannot revoke request in status: ${ar.status}` });
  ar.status = 'revoked';
  ar.decided_at = new Date().toISOString();
  res.json(ar);
});
