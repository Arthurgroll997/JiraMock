...
router.get('/', (req, res) => {
  ...
  if (search) {
    ...
    results = results.filter((a) =>
      `${a.Name} ${a.SerialNumber} ${a.AssetTag} ${a.Manufacturer} ${a.Model}`
        .toLowerCase()
        .includes(s),
    );
  }
...
router.delete('/:id/software/:softwareId', (req, res) => {
  const idx = store.softwareInstallations.findIndex(
    (si) => si.assetId === req.params.id && si.softwareId === req.params.softwareId,
  );
...
router.post('/:id/unassign', (req, res) => {
  ...
  const idx = store.assetAssignments.findIndex(
    (a) => a.assetId === req.params.id && a.status === 'Active',
  );
...
router.get('/:id/compliance', (req, res) => {
  ...
  if (!compliance)
    return res.json({ assetId: req.params.id, status: 'Unknown', complianceScore: null });
  res.json(compliance);
});
