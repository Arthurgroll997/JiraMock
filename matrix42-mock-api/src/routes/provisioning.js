...
router.post('/workflows', (req, res) => {
  ...
  const wf = {
    ...
    type,
    userId,
    userName: user ? user.AccountName : userId,
    status: 'pending',
...
router.get('/workflows/:id', (req, res) => {
  ...
  const steps = store.workflowSteps
    .filter((s) => s.workflowId === req.params.id)
...
router.post('/workflows/:id/execute', (req, res) => {
  ...
  if (wf.status === 'completed' || wf.status === 'cancelled')
    return res.status(400).json({ error: `Workflow is ${wf.status}` });
...
router.post('/workflows/:id/steps/:stepId/complete', (req, res) => {
  const step = store.workflowSteps.find(
    (s) => s.id === req.params.stepId && s.workflowId === req.params.id,
  );
