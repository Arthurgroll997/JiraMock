  ...
  if (search) {
    ...
    result = result.filter(
      (u) =>
        ...
        (u.personalDetails.email || '').toLowerCase().includes(q),
    );
  }
  if (UserType) result = result.filter((u) => u.userType === UserType);
  if (ComponentUser !== undefined)
    result = result.filter((u) => u.componentUser === (ComponentUser === 'true'));
  if (filter) {
...
router.post('/', (req, res) => {
  ...
  if (!username)
    return res.status(400).json({ ErrorCode: 'PASWS003E', ErrorMessage: 'username is required' });
  if (db.users.find((u) => u.username === username))
    return res
      .status(409)
      .json({ ErrorCode: 'PASWS007E', ErrorMessage: `User ${username} already exists` });
  const user = {
    id: Math.max(...db.users.map((u) => u.id)) + 1,
    username,
    source: 'CyberArk',
    userType: userType || 'EPVUser',
    componentUser: false,
    vaultAuthorization: vaultAuthorization || [],
    location: location || '\\Users',
    personalDetails: personalDetails || {},
    enableUser: enableUser !== false,
    suspended: false,
