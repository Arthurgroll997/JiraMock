...
router.get('/:id', (req, res) => {
  ...
  if (!group)
    return res.status(404).json({ ErrorCode: 'PASWS016E', ErrorMessage: 'Group not found' });
  res.json(group);
});
...
router.post('/', (req, res) => {
  ...
  if (!groupName)
    return res.status(400).json({ ErrorCode: 'PASWS003E', ErrorMessage: 'groupName is required' });
  if (db.groups.find((g) => g.groupName === groupName))
    return res
      .status(409)
      .json({ ErrorCode: 'PASWS007E', ErrorMessage: `Group ${groupName} already exists` });
  const group = {
    id: Math.max(...db.groups.map((g) => g.id), 0) + 1,
    groupName,
    description: description || '',
    groupType: groupType || 'Vault',
...
router.post('/:id/Members', (req, res) => {
  ...
  if (!memberId && !memberName)
    return res
      .status(400)
      .json({ ErrorCode: 'PASWS003E', ErrorMessage: 'memberId or memberName required' });

  let user;
  if (memberId) user = db.users.find((u) => u.id === Number(memberId));
  else user = db.users.find((u) => u.username === memberName);
  if (!user)
    return res.status(404).json({ ErrorCode: 'PASWS015E', ErrorMessage: 'User not found' });

  if (group.members.find((m) => m.id === user.id))
    return res.status(409).json({ ErrorCode: 'PASWS007E', ErrorMessage: 'User already a member' });
...
router.delete('/:id/Members/:memberId', (req, res) => {
  ...
  if (idx === -1)
    return res
      .status(404)
      .json({ ErrorCode: 'PASWS015E', ErrorMessage: 'Member not found in group' });
