...
router.get('/', (req, res) => {
  ...
  if (search) {
    ...
    result = result.filter(
      (p) => p.PlatformName.toLowerCase().includes(q) || p.PlatformID.toLowerCase().includes(q),
    );
  }
  if (Active !== undefined) {
...
router.get('/:platformId', (req, res) => {
  ...
  if (!p)
    return res.status(404).json({ ErrorCode: 'PASWS014E', ErrorMessage: 'Platform not found' });
  res.json(p);
});
