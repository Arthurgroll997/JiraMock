...
router.get('/', (req, res) => {
  ...
  if (search) {
    ...
    result = result.filter(
      (s) =>
        s.SafeName.toLowerCase().includes(q) || (s.Description || '').toLowerCase().includes(q),
    );
  }
...
router.get('/:safeUrlId', (req, res) => {
  ...
  if (!safe)
    return res
      .status(404)
      .json({ ErrorCode: 'PASWS012E', ErrorMessage: `Safe ${req.params.safeUrlId} not found` });
  res.json(safe);
});
...
router.post('/', (req, res) => {
  ...
  if (!SafeName)
    return res.status(400).json({ ErrorCode: 'PASWS003E', ErrorMessage: 'SafeName is required' });
  if (db.safes.find((s) => s.SafeUrlId === SafeName))
    return res
      .status(409)
      .json({ ErrorCode: 'PASWS007E', ErrorMessage: `Safe ${SafeName} already exists` });
  const safe = {
    SafeUrlId: SafeName,
    SafeName,
    Description: Description || '',
    ManagingCPM: ManagingCPM || 'PasswordManager',
    NumberOfVersionsRetention: NumberOfVersionsRetention || 5,
    NumberOfDaysRetention: NumberOfDaysRetention || 30,
    OLACEnabled: OLACEnabled || false,
    AutoPurgeEnabled: AutoPurgeEnabled || false,
    Location: '\\',
    Creator: { Id: req.userId, Name: req.username },
    CreationTime: (Date.now() / 1000) | 0,
    LastModificationTime: (Date.now() / 1000) | 0,
  };
  db.safes.push(safe);
...
router.post('/:safeUrlId/Members', (req, res) => {
  ...
  if (!MemberName)
    return res.status(400).json({ ErrorCode: 'PASWS003E', ErrorMessage: 'MemberName is required' });
  const existing = db.safeMembers.find(
    (m) => m.SafeUrlId === req.params.safeUrlId && m.MemberName === MemberName,
  );
  if (existing)
    return res
      .status(409)
      .json({
        ErrorCode: 'PASWS007E',
        ErrorMessage: `Member ${MemberName} already exists in safe`,
      });
  const member = {
    SafeUrlId: req.params.safeUrlId,
    MemberId: `m${Date.now()}`,
    MemberName,
...
router.put('/:safeUrlId/Members/:memberName', (req, res) => {
  const member = db.safeMembers.find(
    (m) => m.SafeUrlId === req.params.safeUrlId && m.MemberName === req.params.memberName,
  );
  if (!member)
    return res.status(404).json({ ErrorCode: 'PASWS012E', ErrorMessage: 'Member not found' });
...
router.delete('/:safeUrlId/Members/:memberName', (req, res) => {
  const idx = db.safeMembers.findIndex(
    (m) => m.SafeUrlId === req.params.safeUrlId && m.MemberName === req.params.memberName,
  );
  if (idx === -1)
    return res.status(404).json({ ErrorCode: 'PASWS012E', ErrorMessage: 'Member not found' });
