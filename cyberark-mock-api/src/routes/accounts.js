...
router.get('/:id', (req, res) => {
  ...
  if (!acc)
    return res.status(404).json({ ErrorCode: 'PASWS013E', ErrorMessage: 'Account not found' });
...
router.post('/', (req, res) => {
  ...
  if (!safeName || !platformId || !address || !userName) {
    return res
      ...
      .json({
        ErrorCode: 'PASWS003E',
        ErrorMessage: 'safeName, platformId, address, userName are required',
      });
  }
  if (!db.safes.find((s) => s.SafeUrlId === safeName)) {
    return res
      .status(404)
      .json({ ErrorCode: 'PASWS012E', ErrorMessage: `Safe ${safeName} not found` });
  }
  const acc = {
    ...
    secretManagement: {
      ...
      lastModifiedTime: (Date.now() / 1000) | 0,
    },
