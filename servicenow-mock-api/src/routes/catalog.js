...
const catalogItems = [
  {
    ...
    category: 'Access',
    description: 'Request privileged access to a server via Fudo PAM',
    delivery_time: '2 business days',
  },
  {
    item_id: 'cat-access-revoke',
    name: 'Emergency Access Revocation',
    category: 'Access',
    description: 'Immediately revoke all privileged access for a user',
    delivery_time: '1 hour',
  },
  {
    item_id: 'cat-vault-onboard',
    name: 'Password Vault Onboarding',
    category: 'Security',
    description: 'Onboard service account into Fudo PAM password vault',
    delivery_time: '3 business days',
  },
  {
    item_id: 'cat-new-server',
    name: 'New Server PAM Enrollment',
    category: 'Infrastructure',
    description: 'Enroll a new server into PAM monitoring and session recording',
...
router.post('/items/:item_id/order', (req, res) => {
  ...
  const request = {
    sys_id: reqId,
    number: reqNum,
    short_description: `Order: ${item.name}`,
    description: req.body.description || '',
    state: 1,
    priority: 3,
    requested_for: req.body.requested_for || '',
    opened_by: req.body.opened_by || 'api',
    approval: 'requested',
  ...
  const reqItem = {
    ...
    state: 1,
    catalog_item: req.params.item_id,
    assigned_to: '',
    assignment_group: '',
    stage: 'waiting_for_approval',
...
router.get('/requests/:req_id/status', (req, res) => {
  const request = store.tables.sc_request.find(
    (r) => r.sys_id === req.params.req_id || r.number === req.params.req_id,
  );
