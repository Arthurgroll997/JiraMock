...
router.get('/ci/:sys_id/relations', (req, res) => {
  ...
  const rels = store.tables.cmdb_rel_ci.filter(
    (r) => r.parent === req.params.sys_id || r.child === req.params.sys_id,
  );
  const outbound = rels
    .filter((r) => r.parent === req.params.sys_id)
    .map((r) => ({
      ...r,
      target: store.tables.cmdb_ci_server.find((s) => s.sys_id === r.child),
    }));
  const inbound = rels
    .filter((r) => r.child === req.params.sys_id)
    .map((r) => ({
      ...r,
...
router.post('/ci/:sys_id/relations', (req, res) => {
  ...
  if (!child || !type)
    return res.status(400).json({ error: { message: 'child and type are required' } });
