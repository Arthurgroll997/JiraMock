...
router.get('/:id/users', (req, res) => {
  if (!db.safes.find((s) => s.id === req.params.id))
    return res.status(404).json({ error: 'Not Found', message: 'Safe not found' });
...
router.post('/:id/users', (req, res) => {
  ...
  if (db.safeUsers.find((su) => su.safe_id === req.params.id && su.user_id === user_id))
    return res.status(409).json({ error: 'Conflict', message: 'User already assigned' });
...
router.delete('/:id/users/:user_id', (req, res) => {
  const idx = db.safeUsers.findIndex(
    (su) => su.safe_id === req.params.id && su.user_id === req.params.user_id,
  );
  if (idx === -1)
...
router.get('/:id/accounts', (req, res) => {
  if (!db.safes.find((s) => s.id === req.params.id))
    return res.status(404).json({ error: 'Not Found', message: 'Safe not found' });
  const accountIds = db.safeAccounts
    .filter((sa) => sa.safe_id === req.params.id)
    .map((sa) => sa.account_id);
...
router.post('/:id/accounts', (req, res) => {
  ...
  if (!account_id || !db.accounts.find((a) => a.id === account_id))
    return res
      .status(422)
      .json({ error: 'Validation Error', message: 'Valid account_id required' });
  if (db.safeAccounts.find((sa) => sa.safe_id === req.params.id && sa.account_id === account_id))
    return res.status(409).json({ error: 'Conflict', message: 'Account already in safe' });
...
router.delete('/:id/accounts/:account_id', (req, res) => {
  const idx = db.safeAccounts.findIndex(
    (sa) => sa.safe_id === req.params.id && sa.account_id === req.params.account_id,
  );
  if (idx === -1)
    return res.status(404).json({ error: 'Not Found', message: 'Assignment not found' });
