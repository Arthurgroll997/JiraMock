...
router.post('/', (req, res) => {
  ...
  if (!user_id || (!safe_id && !account_id) || !justification) {
    return res
      ...
      .json({
        error: 'Bad Request',
        message: 'user_id, safe_id or account_id, and justification are required',
      });
  }
  ...
  const request = {
    ...
    target: safe_id
      ? (db.safes.find((s) => s.id === safe_id) || {}).name || safe_id
      : (db.accounts.find((a) => a.id === account_id) || {}).name || account_id,
    justification,
...
router.post('/:id/approve', (req, res) => {
  ...
  if (!r) return res.status(404).json({ error: 'Not Found', message: 'Access request not found' });
  if (r.status !== 'pending')
    return res.status(409).json({ error: 'Conflict', message: `Request is ${r.status}` });
  const now = new Date();
  r.status = 'approved';
  ...
  if (db.events) {
    db.events.push({
      ...
      details: { request_id: r.id, user: r.requester, target: r.target, approved_by: r.decided_by },
    });
  }
  res.json(r);
});
...
router.post('/:id/revoke', (req, res) => {
  ...
  if (r.status !== 'approved')
    return res
      .status(409)
      .json({ error: 'Conflict', message: 'Only approved requests can be revoked' });
  r.status = 'revoked';
  r.decided_at = new Date().toISOString();
  ...
  if (db.events) {
    db.events.push({
      ...
      type: 'access.denied',
      timestamp: r.decided_at,
      details: { request_id: r.id, user: r.requester, target: r.target, action: 'revoked' },
    });
  }
  res.json(r);
});
