...
router.get('/', (req, res) => {
  ...
  res.json({
    total: db.listeners.length,
    limit,
    offset,
    items: db.listeners.slice(offset, offset + limit),
  });
});
...
router.post('/', (req, res) => {
  ...
  if (!name || !port)
    return res
      .status(422)
      .json({ error: 'Validation Error', message: 'name and port are required' });
...
router.delete('/:id', (req, res) => {
  ...
  if (idx === -1)
    return res.status(404).json({ error: 'Not Found', message: 'Listener not found' });
