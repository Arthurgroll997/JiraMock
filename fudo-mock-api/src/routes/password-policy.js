...
router.post('/', (req, res) => {
  ...
  const policy = {
    ...
    require_special: require_special !== false,
    accounts: accounts || [],
...
router.delete('/:id', (req, res) => {
  ...
  if (idx === -1)
    return res.status(404).json({ error: 'Not Found', message: 'Password policy not found' });
  ...
  const rotated = p.accounts.map((account_id) => {
    ...
    if (db.events) {
      db.events.push({
        ...
        timestamp: now,
        details: { policy: p.name, account: account ? account.name : account_id },
      });
    }
    return {
      account_id,
