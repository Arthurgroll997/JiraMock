...
router.post('/connect', (req, res) => {
  ...
  if (!user_id || !account_id || !protocol) {
    return res
      .status(400)
      .json({ error: 'Bad Request', message: 'user_id, account_id, and protocol are required' });
  }
...
router.post('/:session_id/terminate', (req, res) => {
  ...
  if (session.status === 'terminated' || session.status === 'completed') {
    return res
      .status(409)
      .json({ error: 'Conflict', message: `Session already ${session.status}` });
  }
  const terminated_at = new Date().toISOString();
  session.status = 'terminated';
  session.ended_at = terminated_at;
  session.duration_seconds = Math.floor(
    (new Date(terminated_at) - new Date(session.started_at)) / 1000,
  );
  ...
  if (db.events) {
    db.events.push({
      ...
      details: {
        session_id: session.id,
        terminated_by: req.body.terminated_by || 'admin',
        reason: req.body.reason || 'Manual termination',
      },
    });
  }
...
router.post('/:session_id/resume', (req, res) => {
  ...
  if (session.status !== 'paused')
    return res.status(409).json({ error: 'Conflict', message: 'Session is not paused' });
...
router.get('/:session_id/summary', (req, res) => {
  ...
  const summaries = {
    ssh: {
      summary:
        'User performed system maintenance operations including log rotation and service restarts.',
      commands_executed: [
      ...
      files_transferred: ['backup.sql (245 MB)'],
    },
    rdp: {
      summary:
        'User accessed Active Directory management console and performed group policy updates.',
      commands_executed: [
        'gpupdate /force',
        'Get-ADUser -Filter *',
        'New-ADGroup -Name "Temp-Access"',
      ],
      files_transferred: [],
    },
    vnc: {
      summary: 'User accessed graphical desktop for application monitoring.',
      commands_executed: [],
      files_transferred: [],
    },
    http: {
      summary: 'User accessed web management interface for configuration changes.',
      commands_executed: [],
      files_transferred: ['config-export.json (12 KB)'],
    },
  };
  ...
  res.json({
    session_id: session.id,
    summary: s.summary,
    risk_score: +(Math.random() * 0.4).toFixed(2),
