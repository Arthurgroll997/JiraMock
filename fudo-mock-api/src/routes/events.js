...
router.post('/webhooks', (req, res) => {
  ...
  if (!url || !events)
    return res.status(400).json({ error: 'Bad Request', message: 'url and events are required' });
  const webhook = {
    id: uuidv4(),
    url,
    events,
...
router.post('/webhooks/:id/test', (req, res) => {
  ...
  const testEvent = {
    ...
    details: { message: 'Test event from Fudo PAM' },
    test: true,
  };
...
router.get('/stream', (req, res) => {
  ...
  const interval = setInterval(() => {
    ...
    const event = {
      ...
      type,
      timestamp: new Date().toISOString(),
      details: { message: `Simulated ${type} event` },
    };
