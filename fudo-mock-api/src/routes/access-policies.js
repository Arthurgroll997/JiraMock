...
router.post('/', (req, res) => {
  ...
  if (!group_id)
    return res.status(422).json({ error: 'Validation Error', message: 'group_id is required' });
  if (!safe_id)
    return res.status(422).json({ error: 'Validation Error', message: 'safe_id is required' });

  // Validate references
  if (!db.groups.find((g) => g.id === group_id))
    return res.status(422).json({ error: 'Validation Error', message: 'Group not found' });
  if (!db.safes.find((s) => s.id === safe_id))
    return res.status(422).json({ error: 'Validation Error', message: 'Safe not found' });
  if (listener_id && !db.listeners.find((l) => l.id === listener_id))
    return res.status(422).json({ error: 'Validation Error', message: 'Listener not found' });
  ...
  const policy = {
    ...
    listener_name: listener_id
      ? db.listeners.find((l) => l.id === listener_id)?.name || null
      : null,
...
router.put('/:id', (req, res) => {
  ...
  if (p.listener_id)
    p.listener_name = db.listeners.find((l) => l.id === p.listener_id)?.name || null;
...
router.delete('/:id', (req, res) => {
  ...
  if (idx === -1)
    return res.status(404).json({ error: 'Not Found', message: 'Access policy not found' });
...
router.get('/check/:user_id/:safe_id', (req, res) => {
  ...
  const userGroupIds = db.groupUsers
    .filter((gu) => gu.user_id === user_id)
    .map((gu) => gu.group_id);

  // Find policies that match group + safe
  const policies = db.accessPolicies.filter(
    (p) => p.safe_id === safe_id && userGroupIds.includes(p.group_id) && p.status === 'active',
  );
