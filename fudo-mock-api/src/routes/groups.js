...
router.post('/', (req, res) => {
  ...
  if (!name)
    return res.status(422).json({ error: 'Validation Error', message: 'name is required' });
  const now = new Date().toISOString();
  const group = {
    id: uuidv4(),
    name,
    description: description || '',
    ad_group_dn: ad_group_dn || null,
    created_at: now,
    modified_at: now,
  };
...
router.delete('/:id', (req, res) => {
  ...
  db.groupUsers.splice(
    0,
    db.groupUsers.length,
    ...db.groupUsers.filter((gu) => gu.group_id !== req.params.id),
  );
  db.groupSafes.splice(
    0,
    db.groupSafes.length,
    ...db.groupSafes.filter((gs) => gs.group_id !== req.params.id),
  );
  res.status(204).end();
});
...
router.get('/:id/users', (req, res) => {
  if (!db.groups.find((g) => g.id === req.params.id))
    return res.status(404).json({ error: 'Not Found', message: 'Group not found' });
  const userIds = db.groupUsers
    .filter((gu) => gu.group_id === req.params.id)
    .map((gu) => gu.user_id);
...
router.post('/:id/users', (req, res) => {
  ...
  if (db.groupUsers.find((gu) => gu.group_id === req.params.id && gu.user_id === user_id))
    return res.status(409).json({ error: 'Conflict', message: 'User already in group' });
...
router.delete('/:id/users/:user_id', (req, res) => {
  const idx = db.groupUsers.findIndex(
    (gu) => gu.group_id === req.params.id && gu.user_id === req.params.user_id,
  );
  if (idx === -1)
...
router.get('/:id/safes', (req, res) => {
  if (!db.groups.find((g) => g.id === req.params.id))
    return res.status(404).json({ error: 'Not Found', message: 'Group not found' });
  const safeIds = db.groupSafes
    .filter((gs) => gs.group_id === req.params.id)
    .map((gs) => gs.safe_id);
...
router.post('/:id/safes', (req, res) => {
  ...
  if (!safe_id || !db.safes.find((s) => s.id === safe_id))
    return res.status(422).json({ error: 'Validation Error', message: 'Valid safe_id required' });
  if (db.groupSafes.find((gs) => gs.group_id === req.params.id && gs.safe_id === safe_id))
    return res.status(409).json({ error: 'Conflict', message: 'Safe already assigned to group' });
...
router.delete('/:id/safes/:safe_id', (req, res) => {
  const idx = db.groupSafes.findIndex(
    (gs) => gs.group_id === req.params.id && gs.safe_id === req.params.safe_id,
  );
  if (idx === -1)
    return res.status(404).json({ error: 'Not Found', message: 'Assignment not found' });
