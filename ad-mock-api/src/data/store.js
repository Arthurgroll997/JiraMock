...
class Store {
  ...
  // Users
  findUser(sam) {
    return this.users.find((u) => u.sAMAccountName === sam);
  }
  findUserByDN(dn) {
    return this.users.find((u) => u.distinguishedName === dn);
  }

  // Groups
  findGroup(name) {
    return this.groups.find((g) => g.sAMAccountName === name);
  }

  // Computers
  findComputer(name) {
    return this.computers.find((c) => c.cn === name);
  }
