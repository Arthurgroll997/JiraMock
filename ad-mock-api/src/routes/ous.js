...
function buildTree(ous) {
  ...
  const sorted = [...ous].sort(
    (a, b) => a.distinguishedName.split(',').length - b.distinguishedName.split(',').length,
  );
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (!child) {
        child = {
          ...
          name,
          description: i === parts.length - 1 ? ou.description : '',
          children: [],
        };
...
router.get('/:path', (req, res) => {
  ...
  res.json({
    ...
    userList: users.map((u) => u.sAMAccountName),
    groupList: groups.map((g) => g.sAMAccountName),
  });
});
...
router.post('/', (req, res) => {
  ...
  if (store.ous.find((o) => o.distinguishedName === dn))
    return res.status(409).json({ error: 'OU already exists' });
