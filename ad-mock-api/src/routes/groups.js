...
router.get('/', (req, res) => {
  ...
  if (filter) {
    ...
    result = result.filter(
      (g) => g.cn.toLowerCase().includes(f) || g.description?.toLowerCase().includes(f),
    );
  }
...
router.post('/', (req, res) => {
  ...
  const group = {
    distinguishedName: `CN=${name},${groupOU}`,
    cn: name,
    sAMAccountName: name,
...
router.put('/:name', (req, res) => {
  const g = store.findGroup(req.params.name);
  if (!g) return res.status(404).json({ error: 'Group not found' });
  for (const key of ['description', 'managedBy', 'groupScope']) {
    if (req.body[key] !== undefined) g[key] = req.body[key];
  }
...
router.delete('/:name', (req, res) => {
  ...
  for (const u of store.users) {
    u.memberOf = u.memberOf.filter((g) => g !== removed.distinguishedName);
  }
...
router.post('/:name/members', (req, res) => {
  ...
  for (const sam of members) {
    ...
    if (!u) continue;
    if (!g.members.includes(u.distinguishedName)) {
      g.members.push(u.distinguishedName);
    }
    if (!u.memberOf.includes(g.distinguishedName)) {
      u.memberOf.push(g.distinguishedName);
    }
    added.push(sam);
  }
  g.whenChanged = new Date().toISOString();
...
router.post('/:name/members/timed', (req, res) => {
  ...
  res
    .status(201)
    .json({ message: `Timed membership added for ${user} in ${req.params.name}`, expires_at });
});
