...
router.post('/', (req, res) => {
  ...
  if (!sAMAccountName || !cn)
    return res.status(400).json({ error: 'sAMAccountName and cn required' });
  ...
  const user = {
    ...
    sn: sn || '',
    displayName: cn,
    mail: `${sAMAccountName}@corp.local`,
...
router.post('/:sam/groups', (req, res) => {
  ...
  for (const gName of groups) {
    ...
    if (!group) continue;
    if (!group.members.includes(user.distinguishedName)) {
      group.members.push(user.distinguishedName);
      group.whenChanged = new Date().toISOString();
    }
    if (!user.memberOf.includes(group.distinguishedName)) {
      user.memberOf.push(group.distinguishedName);
    }
    added.push(gName);
  }
  user.whenChanged = new Date().toISOString();
