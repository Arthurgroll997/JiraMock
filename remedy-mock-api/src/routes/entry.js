...
function formatEntry(formName, entry, req) {
  ...
  return {
    ...
    _links: {
      self: [
        {
          href: `${req.protocol}://${req.get('host')}/api/arsys/v1/entry/${encodeURIComponent(formName)}/${idField}`,
        },
      ],
    },
  };
...
router.get('/:formName', (req, res) => {
  ...
  if (!form) {
    return res
      ...
      .json({
        error: [
          { messageType: 'ERROR', messageText: `Form ${formName} not found`, messageNumber: 302 },
        ],
      });
  }
  ...
  const result = entries.map((e) => {
    ...
    if (fields) {
      const filtered = {};
      fields.forEach((f) => {
        if (formatted.values[f] !== undefined) filtered[f] = formatted.values[f];
      });
...
router.get('/:formName/:entryId', (req, res) => {
  ...
  if (!entry)
    return res
      .status(404)
      .json({
        error: [{ messageType: 'ERROR', messageText: 'Entry not found', messageNumber: 302 }],
      });
...
router.post('/:formName', (req, res) => {
  ...
  const entry = {
    'Request ID': uuidv4().replace(/-/g, '').slice(0, 15),
  ...
  res
    ...
    .json({
      ...
      _links: {
        self: [
          {
            href: `${req.protocol}://${req.get('host')}/api/arsys/v1/entry/${encodeURIComponent(formName)}/${id}`,
          },
        ],
      },
    });
