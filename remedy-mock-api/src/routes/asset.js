...
router.get('/', (req, res) => {
  ...
  if (req.query.environment)
    entries = entries.filter((e) => e['Environment'] === req.query.environment);
  res.json({
...
router.get('/:id', (req, res) => {
  const entry = store.forms[FORM].find(
    (e) =>
      e['CI Name'] === req.params.id ||
      e['Entry ID'] === req.params.id ||
      e['Asset Tag'] === req.params.id,
  );
  if (!entry)
    return res
      .status(404)
      .json({
        error: [{ messageType: 'ERROR', messageText: 'Asset not found', messageNumber: 302 }],
      });
...
router.get('/:id/topology', (req, res) => {
  const entry = store.forms[FORM].find(
    (e) => e['CI Name'] === req.params.id || e['Entry ID'] === req.params.id,
  );
  if (!entry)
    return res
      .status(404)
  ...
  if (ciName === 'DC01') {
    allAssets
      .filter((a) => a['CI Name'] !== 'DC01')
      .forEach((a) => usedBy.push({ 'CI Name': a['CI Name'], Relationship: 'Used by' }));
  } else {
    dependsOn.push({ 'CI Name': 'DC01', Relationship: 'Depends on' });
  }
  if (ciName === 'FUDO-PAM') {
    allAssets
      .filter((a) => a['CI Name'] !== 'FUDO-PAM')
      .forEach((a) => usedBy.push({ 'CI Name': a['CI Name'], Relationship: 'Manages' }));
  }
  ...
  res.json({
    ...
    relatedIncidents: incidents.map((i) => ({
      'Incident Number': i['Incident Number'],
      Description: i['Description'],
      Status: i['Status'],
    })),
    relatedChanges: changes.map((c) => ({
      'Change Number': c['Change Number'],
