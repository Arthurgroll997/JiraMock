...
router.get('/:id', (req, res) => {
  const entry = store.forms[FORM].find(
    (e) => e['Change Number'] === req.params.id || e['Entry ID'] === req.params.id,
  );
  if (!entry)
    return res
      .status(404)
      .json({
        error: [
          { messageType: 'ERROR', messageText: 'Change request not found', messageNumber: 302 },
        ],
      });
...
function updateChange(id, updates) {
  ...
  Object.assign(entry, updates, {
    'Modified Date': new Date().toISOString().replace('T', ' ').slice(0, 19),
  });
  return entry;
}

router.post('/:id/schedule', (req, res) => {
  const entry = updateChange(req.params.id, {
    Status: 'Scheduled',
    'Scheduled Start Date': req.body.startDate || '',
    'Scheduled End Date': req.body.endDate || '',
  });
  if (!entry)
    return res
      .status(404)
...
router.get('/:id/tasks', (req, res) => {
  ...
  res.json({
    entries: [
      {
        values: {
          'Task ID': 'TAS000000000001',
          Summary: `Pre-implementation checks for ${entry['Change Number']}`,
          Status: 'Assigned',
      ...
      {
        values: {
          'Task ID': 'TAS000000000002',
          Summary: `Execute change ${entry['Change Number']}`,
          Status: 'Pending',
          Assignee: entry['Assignee'],
          Priority: entry['Priority'],
        },
      ...
      {
        values: {
          'Task ID': 'TAS000000000003',
          Summary: `Post-implementation verification for ${entry['Change Number']}`,
          Status: 'Pending',
          Assignee: entry['Assignee'],
          Priority: entry['Priority'],
        },
