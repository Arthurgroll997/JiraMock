...
router.get('/:id', (req, res) => {
  const entry = store.forms[FORM].find(e => e['Work Order ID'] === req.params.id || e['Entry ID'] === req.params.id);
  if (!entry) return res.status(404).json({ error: [{ messageType: 'ERROR', messageText: 'Work order not found', messageNumber: 302 }] });
