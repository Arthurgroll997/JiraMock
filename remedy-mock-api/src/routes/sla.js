...
router.get('/status/:incidentId', (req, res) => {
  const incident = store.forms['HPD:Help Desk'].find(e => e['Incident Number'] === req.params.incidentId || e['Entry ID'] === req.params.incidentId);
  if (!incident) return res.status(404).json({ error: [{ messageType: 'ERROR', messageText: 'Incident not found', messageNumber: 302 }] });

  const sla = store.forms[FORM].find(s => s['Priority'] === incident['Priority']);
  if (!sla) return res.json({ incident: incident['Incident Number'], sla: null, message: 'No SLA definition for this priority' });
  ...
  res.json({
    ...
    response: {
      ...
      status: incident['Status'] === 'New' ? (elapsedMin > responseTarget ? 'Breached' : 'Within SLA') : 'Met'
    },
    resolution: {
      targetMinutes: resolutionTarget,
      elapsedMinutes: elapsedMin,
      breached: elapsedMin > resolutionTarget && !['Resolved', 'Closed'].includes(incident['Status']),
      status: ['Resolved', 'Closed'].includes(incident['Status']) ? 'Met' : (elapsedMin > resolutionTarget ? 'Breached' : 'Within SLA')
    }
  });
});
