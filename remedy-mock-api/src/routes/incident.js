...
router.get('/stats', (req, res) => {
  ...
  const byStatus = {}, byPriority = {}, byGroup = {};
  entries.forEach(e => {
...
router.get('/:id', (req, res) => {
  const entry = store.forms[FORM].find(e => e['Incident Number'] === req.params.id || e['Entry ID'] === req.params.id);
  if (!entry) return res.status(404).json({ error: [{ messageType: 'ERROR', messageText: 'Incident not found', messageNumber: 302 }] });
