...
function authMiddleware(req, res, next) {
  ...
  if (!authHeader) {
    return res
      ...
      .json({
        error: [
          { messageType: 'ERROR', messageText: 'Missing Authorization header', messageNumber: 401 },
        ],
      });
  }
    ...
    return res
      ...
      .json({
        error: [
          {
            messageType: 'ERROR',
            messageText: 'Invalid or expired AR-JWT token',
            messageNumber: 401,
          },
        ],
      });
  ...
  if (authHeader.startsWith('Bearer ')) {
    ...
    return res
      .status(401)
      .json({
        error: [{ messageType: 'ERROR', messageText: 'Invalid Bearer token', messageNumber: 401 }],
      });
  }
  ...
  if (authHeader.startsWith('Basic ')) {
    ...
    if (!validPasswords.includes(password)) {
      return res
        .status(401)
        .json({
          error: [{ messageType: 'ERROR', messageText: 'Invalid credentials', messageNumber: 401 }],
        });
    }
  ...
  return res
    ...
    .json({
      error: [
        {
          messageType: 'ERROR',
          messageText: 'Unsupported authentication method',
          messageNumber: 401,
        },
      ],
    });
